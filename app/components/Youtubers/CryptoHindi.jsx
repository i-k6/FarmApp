...
export default function Cryptohindi() {
  ...
  const fetchVideoList = async () => {
    try {
      const response = await GlobalApi.getCryptoHindi();
      setVideoList(response?.cryptohindis || []);
    } catch (error) {
      ...
      <TouchableOpacity onPress={() => handlePress(item)}>
        <View style={styles.card}>
          {item.thumbnail && item.thumbnail.length > 0 && (
            <Image source={{ uri: item.thumbnail[0].url }} style={styles.thumbnail} />
          )}
